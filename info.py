...
API_ID = int(environ.get('API_ID', '20110837')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', 'b9658b136c2b71af2bdb7497649ace5c')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "8534382621:AAF9j7BsTPWow7R6rScVfewCPMp5gr5o-tM")    # Bot token from @BotFather
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7307633923').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003301313071').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003400814772'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1003400814772'))  # Bin channel id (make sure bot is admin)
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1003129758081')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002692055617')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/filestorekr')  # Support group link (make sure bot is admin)

# FORCE_SUB 
auth_req_channels = environ.get("AUTH_REQ_CHANNELS", "-1002692055617")# requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
auth_channels     = environ.get("AUTH_CHANNELS", "-1002692055617")# Channels for force sub (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://4yxduh8_db_user:45Lyw2zgcCUhxTQd@cluster0.afxbyeo.mongodb.net/?retryWrites=true&w=majority")  # MongoDB URI for the database
...
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1002692055617'))  # Notification of sent to your channel
